...
def get_args_parser():
    ...
    parser.add_argument('--bsp_resume', default='',
...
def main():
    ...
    if args.bsp_resume:
        bsp_load_ckpt(model_without_ddp, args.bsp_resume)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1 == args.epochs or epoch % 20==0) and misc.is_main_process():
            if epoch + 1 == args.epochs:
                ...
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch, exp=exp)
...
def bsp_load_ckpt(model, ckpt_path):
    ...
    assert len(set(diff_keys)) == len(set(sd_before.keys()))-len(set(poped_key.keys())) -2 - len(set(decoder_ckpt.keys()))
