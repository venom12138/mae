...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_metrics = train_one_epoch(
            model, train_loader,
            optimizer, device, epoch, loss_scaler, exp=exp,
            args=args
        )
        if epoch % 20 == 0 or epoch + 1 == args.epochs:
            save_checkpoint({
                ...
                'args': args,
            }, checkpoint=exp.save_dir, file_name=f'checkpoint-{epoch}.pth.tar')
        exp.write(phase, train_metrics=train_metrics, epoch=epoch,lr=optimizer.param_groups[0]['lr'])
