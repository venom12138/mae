...
for repeat in {0,1,2}; do
    for times in {0,1,}; do
        ...
        then
            ...
            bash base.sh --bsp_resume $prefix/*times=$repeat,bsp=$(($times-1))*/checkpoint.pth --epochs 20
        else
            export run_name="times=$repeat,bsp=$times,warmup_epochs=$warmup_epochs,phase=pretrain,norm_pix_loss=1,blr=$blr,bs=$((batch_size * accum_iter))"
            bash base.sh --epochs 180
        fi
    done
done
wait

bash 0526_finetune_bsp_2_ep180_20.sh
bash 0526_linprobe_bsp_2_ep180_20.sh
bash zombie.sh
