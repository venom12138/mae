...
export exp_name=0525_MAE_bsp_2_normpix=0
...
for repeat in {0,1,2}; do
    for times in {1,}; do
        export exp_args="--phase $phase --accum_iter $accum_iter --blr $blr --batch_size $batch_size --warmup_epochs $warmup_epochs --epochs $epochs "
        
        if test $times -gt 0
        then
            export run_name="times=$repeat,bsp=$times,warmup_epochs=$warmup_epochs,phase=pretrain,norm_pix_loss=0,blr=$blr,bs=$((batch_size * accum_iter))"
            ...
            bash base.sh --norm_pix_loss
        fi
    done
done
wait

bash 0525_finetune_bsp_2.sh
bash 0525_linprobe_bsp_2.sh
bash zombie.sh
