...
bash base.sh --finetune $prefix/*bsp=1,warmup_epochs=5,norm_pix_loss=1,phase=pretrain,blr=6.25e-5,bs=1024/checkpoint.pth

# export run_name="norm_pix_loss=1,phase=$phase,blr=$blr,bs=$batch_size"
# bash base.sh --finetune "$prefix/*times=1,warmup_epochs=5,norm_pix_loss=1,phase=pretrain,blr=1.5e-4,bs=1024/checkpoint.pth"
# export run_name="norm_pix_loss=1,phase=$phase,blr=$blr,bs=$batch_size"
# bash base.sh --finetune "$prefix/*times=2,warmup_epochs=5,norm_pix_loss=1,phase=pretrain,blr=1.5e-4,bs=1024/checkpoint.pth"