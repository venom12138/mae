...
for repeat in {0,1,2}; do
    for times in {0,1,}; do
        ...
        if test $times -gt 0
        then
            bash base.sh --bsp_resume $prefix/*times=$repeat,bsp=$(($times-1))*/checkpoint.pth --epochs 50
        else
            bash base.sh --epochs 150
        fi
    done
done
wait

bash 0525_linprobe_bsp_2_ep150+50.sh
bash 0525_finetune_bsp_2_ep150+50.sh
bash zombie.sh
