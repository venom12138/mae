...
CUDA_VISIBLE_DEVICES=0,1 bash base.sh --finetune $prefix/*times=0,bsp=2,warmup_epochs=5,norm_pix_loss=1,phase=pretrain,*/checkpoint.pth &

export MASTER_PORT=25759
export run_name="norm_pix_loss=1,phase=$phase,blr=$blr,bs=$batch_size"
CUDA_VISIBLE_DEVICES=2,3 bash base.sh --finetune $prefix/*times=1,bsp=2,warmup_epochs=5,norm_pix_loss=1,phase=pretrain,*/checkpoint.pth
wait
export MASTER_PORT=25783
export run_name="norm_pix_loss=1,phase=$phase,blr=$blr,bs=$batch_size"
CUDA_VISIBLE_DEVICES=0,1 bash base.sh --finetune $prefix/*times=2,bsp=2,warmup_epochs=5,norm_pix_loss=1,phase=pretrain,*/checkpoint.pth &
